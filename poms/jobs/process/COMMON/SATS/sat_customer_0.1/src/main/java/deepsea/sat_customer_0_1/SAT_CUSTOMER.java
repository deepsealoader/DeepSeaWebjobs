...
package deepsea.sat_customer_0_1;
...
/**
 * Job: SAT_CUSTOMER Purpose: <br>
 * Description:  <br>
 * @author romualdrousseau@gmail.com
 * @version 7.0.1.20180411_1414
 * @status 
 */
public class SAT_CUSTOMER implements TalendJob {
...
public String getPOSTGRES_HOSTNAME(){
	...
	private final String jobName = "SAT_CUSTOMER";
...
private RunStat runStat = new RunStat();
	...
	MetterCatcherUtils tFlowMeterCatcher_1 = new MetterCatcherUtils("_GlquAM6rEeiznP0ccyyRWw", "0.1");
		...
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(SAT_CUSTOMER.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row11Struct implements routines.system.IPersistableRow<row11Struct> {
    final static byte[] commonByteArrayLock_DEEPSEA_SAT_CUSTOMER = new byte[0];
    static byte[] commonByteArray_DEEPSEA_SAT_CUSTOMER = new byte[0];
			    ...
			    public float geographyCoef;

				public float getGeographyCoef () {
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_DEEPSEA_SAT_CUSTOMER.length) {
				if(length < 1024 && commonByteArray_DEEPSEA_SAT_CUSTOMER.length == 0) {
   					commonByteArray_DEEPSEA_SAT_CUSTOMER = new byte[1024];
				} else {
   					commonByteArray_DEEPSEA_SAT_CUSTOMER = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_DEEPSEA_SAT_CUSTOMER, 0, length);
			strReturn = new String(commonByteArray_DEEPSEA_SAT_CUSTOMER, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_DEEPSEA_SAT_CUSTOMER) {
        	...
        	try {
        		...
        		int length = 0;
			        ...
			        this.geographyCoef = dis.readFloat();
					
			        this.splitNumber = dis.readFloat();
    ...
    public void writeData(ObjectOutputStream dos) {
        try {
					...
					// float
		            	...
		            	dos.writeFloat(this.geographyCoef);
					
					// float
				
		            	dos.writeFloat(this.splitNumber);
    ...
    public String toString() {
		...
		sb.append(",geographyCoef="+String.valueOf(geographyCoef));
		sb.append(",splitNumber="+String.valueOf(splitNumber));
...
int count_tDBOutput_1=0;
	    String insert_tDBOutput_1 = "INSERT INTO \"" + tableName_tDBOutput_1 + "\" (\"hub_id\",\"source\",\"code\",\"geography_code\",\"source_channel_code\",\"name_en\",\"name_lo\",\"geography_coef\",\"split_number\",\"attribute1\",\"attribute2\",\"attribute3\",\"attribute4\",\"attribute5\") VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
 ...
 */
	...
	class RowSetValueUtil_tSchemaComplianceCheck_1 {
		...
		void setRowValue_0(row10Struct row9) {
	...
	if ((row9.source == null) || ("".equals(row9.source))) {
		...
		if(
		row9.providerCode != null
		&& (!"".equals(row9.providerCode))
		) {
			String tester_tSchemaComplianceCheck_1 = String.valueOf(row9.providerCode);
		}
	} catch(java.lang.Exception e) {
		ifPassedThrough = false;
		errorCodeThrough += 2;
		errorMessageThrough += "|wrong type";
			...
			resultErrorCodeThrough = handleErrorCode(errorCodeThrough,resultErrorCodeThrough);
			errorCodeThrough = 0;
			resultErrorMessageThrough = handleErrorMessage(errorMessageThrough,resultErrorMessageThrough,"providerCode:");
			errorMessageThrough = "";
	// validate nullable (empty as null)
	if ((row9.geographyCode == null) || ("".equals(row9.geographyCode))) {
		ifPassedThrough = false;
		errorCodeThrough += 4;
		errorMessageThrough += "|empty or null";
	}	try {
	...
	if ((row9.sourceChannelCode == null) || ("".equals(row9.sourceChannelCode))) {
		...
		errorMessageThrough += "|wrong type";
			...
			errorMessageThrough = "";
	// validate nullable (empty as null)
	if ((row9.nameEn == null) || ("".equals(row9.nameEn))) {
		ifPassedThrough = false;
		errorCodeThrough += 4;
		errorMessageThrough += "|empty or null";
	}	try {
		...
		row9.nameEn != null
			...
			String tester_tSchemaComplianceCheck_1 = String.valueOf(row9.nameEn);
		}
	} catch(java.lang.Exception e) {
		ifPassedThrough = false;
		errorCodeThrough += 2;
		errorMessageThrough += "|wrong type";
			...
			resultErrorCodeThrough = handleErrorCode(errorCodeThrough,resultErrorCodeThrough);
			errorCodeThrough = 0;
			resultErrorMessageThrough = handleErrorMessage(errorMessageThrough,resultErrorMessageThrough,"nameEn:");
			errorMessageThrough = "";	try {
		if(
		row9.nameLo != null
		&& (!"".equals(row9.nameLo))
		) {
			String tester_tSchemaComplianceCheck_1 = String.valueOf(row9.nameLo);
		}
	} catch(java.lang.Exception e) {
		ifPassedThrough = false;
		errorCodeThrough += 2;
		errorMessageThrough += "|wrong type";
			...
			resultErrorCodeThrough = handleErrorCode(errorCodeThrough,resultErrorCodeThrough);
			errorCodeThrough = 0;
			resultErrorMessageThrough = handleErrorMessage(errorMessageThrough,resultErrorMessageThrough,"nameLo:");
			errorMessageThrough = "";
			resultErrorCodeThrough = handleErrorCode(errorCodeThrough,resultErrorCodeThrough);
			errorCodeThrough = 0;
			resultErrorMessageThrough = handleErrorMessage(errorMessageThrough,resultErrorMessageThrough,"geographyCoef:");
			errorMessageThrough = "";
			resultErrorCodeThrough = handleErrorCode(errorCodeThrough,resultErrorCodeThrough);
			errorCodeThrough = 0;
			resultErrorMessageThrough = handleErrorMessage(errorMessageThrough,resultErrorMessageThrough,"splitNumber:");
			errorMessageThrough = "";	try {
		if(
 ...
 */
    ...
    row3.geographyCoef = utility_tExtractFields_1.UnPivot(doc_tExtractFields_1, "tag", "value", "geographyCoef");
    row3.splitNumber = utility_tExtractFields_1.UnPivot(doc_tExtractFields_1, "tag", "value", "splitNumber");
...
copyOfrow4_tmp.servierCode = row3.servierCode;
copyOfrow4_tmp.providerCode = row3.providerCode;
copyOfrow4_tmp.geographyCode = TalendString.talendTrim((row3.geographyCode),';',0) ;
copyOfrow4_tmp.sourceChannelCode = row3.sourceChannelCode;
copyOfrow4_tmp.nameEn = row3.nameEn;
copyOfrow4_tmp.nameLo = row3.nameLo;
copyOfrow4_tmp.geographyCoef = row3.geographyCoef;
copyOfrow4_tmp.splitNumber = row3.splitNumber;
copyOfrow4_tmp.attribute1 = row3.attribute1;
copyOfrow4_tmp.attribute2 = row3.attribute2;
copyOfrow4_tmp.attribute3 = row3.attribute3;
copyOfrow4_tmp.attribute4 = row3.attribute4;
copyOfrow4_tmp.attribute5 = row3.attribute5;
...
if(copyOfrow4 != null) { 
	...
	currentComponent="tNormalize_1";
            ...
            if(copyOfrow4.geographyCode != null) {
				if("".equals(copyOfrow4.geographyCode)){
		        	normalizeRecord_tNormalize_1[0] = "";
		        }else{
	                
	                		normalizeRecord_tNormalize_1 = copyOfrow4.geographyCode.split(";",-1);
            	...
            	for(int i_tNormalize_1 = 0 ; i_tNormalize_1 < lastNoEmptyIndex_tNormalize_1 ; i_tNormalize_1++) {
	                ...
	                nb_line_tNormalize_1++;               
	                                	...
	                                	tmp_tNormalize_1 = copyOfrow4.servierCode == null ? null : String.valueOf(copyOfrow4.servierCode);
	                                
		                            if(tmp_tNormalize_1 != null){
		                                currentRecord_tNormalize_1.append(tmp_tNormalize_1 + tmp_tNormalize_1.length());
		                            }
	                            
	                                	tmp_tNormalize_1 = copyOfrow4.providerCode == null ? null : String.valueOf(copyOfrow4.providerCode);
		                            ...
		                            if(normalizeRecord_tNormalize_1[i_tNormalize_1] != null) {
	                                	...
	                                	tmp_tNormalize_1 = copyOfrow4.sourceChannelCode == null ? null : String.valueOf(copyOfrow4.sourceChannelCode);
	                                
		                            if(tmp_tNormalize_1 != null){
		                                currentRecord_tNormalize_1.append(tmp_tNormalize_1 + tmp_tNormalize_1.length());
		                            }
	                            
	                                	tmp_tNormalize_1 = copyOfrow4.nameEn == null ? null : String.valueOf(copyOfrow4.nameEn);
	                                
		                            if(tmp_tNormalize_1 != null){
		                                currentRecord_tNormalize_1.append(tmp_tNormalize_1 + tmp_tNormalize_1.length());
		                            }
	                            
	                                	tmp_tNormalize_1 = copyOfrow4.nameLo == null ? null : String.valueOf(copyOfrow4.nameLo);
	                                
		                            if(tmp_tNormalize_1 != null){
		                                currentRecord_tNormalize_1.append(tmp_tNormalize_1 + tmp_tNormalize_1.length());
		                            }
	                            
	                                	tmp_tNormalize_1 = copyOfrow4.geographyCoef == null ? null : String.valueOf(copyOfrow4.geographyCoef);
	                                
		                            if(tmp_tNormalize_1 != null){
		                                currentRecord_tNormalize_1.append(tmp_tNormalize_1 + tmp_tNormalize_1.length());
		                            }
	                            
	                                	tmp_tNormalize_1 = copyOfrow4.splitNumber == null ? null : String.valueOf(copyOfrow4.splitNumber);
	                                
		                            if(tmp_tNormalize_1 != null){
		                                currentRecord_tNormalize_1.append(tmp_tNormalize_1 + tmp_tNormalize_1.length());
		                            }
	                            
	                                	tmp_tNormalize_1 = copyOfrow4.attribute1 == null ? null : String.valueOf(copyOfrow4.attribute1);
	                                
		                            if(tmp_tNormalize_1 != null){
		                                currentRecord_tNormalize_1.append(tmp_tNormalize_1 + tmp_tNormalize_1.length());
		                            }
	                            
	                                	tmp_tNormalize_1 = copyOfrow4.attribute2 == null ? null : String.valueOf(copyOfrow4.attribute2);
	                                
		                            if(tmp_tNormalize_1 != null){
		                                currentRecord_tNormalize_1.append(tmp_tNormalize_1 + tmp_tNormalize_1.length());
		                            }
	                            
	                                	tmp_tNormalize_1 = copyOfrow4.attribute3 == null ? null : String.valueOf(copyOfrow4.attribute3);
	                                
		                            if(tmp_tNormalize_1 != null){
		                                currentRecord_tNormalize_1.append(tmp_tNormalize_1 + tmp_tNormalize_1.length());
		                            }
	                            
	                                	tmp_tNormalize_1 = copyOfrow4.attribute4 == null ? null : String.valueOf(copyOfrow4.attribute4);
	                                
		                            if(tmp_tNormalize_1 != null){
		                                currentRecord_tNormalize_1.append(tmp_tNormalize_1 + tmp_tNormalize_1.length());
		                            }
	                            
	                                	tmp_tNormalize_1 = copyOfrow4.attribute5 == null ? null : String.valueOf(copyOfrow4.attribute5);
		                    ...
		                    if(!recordSet_tNormalize_1.contains(currentRecord_tNormalize_1.toString())) {
	                        	...
	                        	row13.servierCode = copyOfrow4.servierCode;
	                        
	                        	row13.providerCode = copyOfrow4.providerCode;
	                        
	                        	row13.geographyCode = normalizeRecord_tNormalize_1[i_tNormalize_1];
		                    
	                        	row13.sourceChannelCode = copyOfrow4.sourceChannelCode;
	                        
	                        	row13.nameEn = copyOfrow4.nameEn;
	                        
	                        	row13.nameLo = copyOfrow4.nameLo;
	                        
	                        	row13.geographyCoef = copyOfrow4.geographyCoef;
	                        
	                        	row13.splitNumber = copyOfrow4.splitNumber;
	                        
	                        	row13.attribute1 = copyOfrow4.attribute1;
	                        
	                        	row13.attribute2 = copyOfrow4.attribute2;
	                        
	                        	row13.attribute3 = copyOfrow4.attribute3;
	                        
	                        	row13.attribute4 = copyOfrow4.attribute4;
	                        
	                        	row13.attribute5 = copyOfrow4.attribute5;
 ...
 */
  ...
  boolean bHasError_tConvertType_1 = false;             
          try {
              if ("".equals(row13.servierCode)){  
                row13.servierCode = null;
              }
              row2.servierCode=TypeConvert.String2String(row13.servierCode);            
          } catch(java.lang.Exception e){
            bHasError_tConvertType_1 = true;            
              System.err.println(e.getMessage());          
          }           
          try {
              if ("".equals(row13.providerCode)){  
                row13.providerCode = null;
              }
              row2.providerCode=TypeConvert.String2String(row13.providerCode);            
          } catch(java.lang.Exception e){
            bHasError_tConvertType_1 = true;            
              System.err.println(e.getMessage());          
          }           
          try {
              if ("".equals(row13.geographyCode)){  
                row13.geographyCode = null;
              }
              row2.geographyCode=TypeConvert.String2String(row13.geographyCode);            
          } catch(java.lang.Exception e){
            bHasError_tConvertType_1 = true;            
              System.err.println(e.getMessage());          
          }           
          try {
              if ("".equals(row13.sourceChannelCode)){  
                row13.sourceChannelCode = null;
              }
              row2.sourceChannelCode=TypeConvert.String2String(row13.sourceChannelCode);            
          } catch(java.lang.Exception e){
            bHasError_tConvertType_1 = true;            
              System.err.println(e.getMessage());          
          }           
          try {
              if ("".equals(row13.nameEn)){  
                row13.nameEn = null;
              }
              row2.nameEn=TypeConvert.String2String(row13.nameEn);            
          } catch(java.lang.Exception e){
            bHasError_tConvertType_1 = true;            
              System.err.println(e.getMessage());          
          }           
          try {
              if ("".equals(row13.nameLo)){  
                row13.nameLo = null;
              }
              row2.nameLo=TypeConvert.String2String(row13.nameLo);            
          } catch(java.lang.Exception e){
            bHasError_tConvertType_1 = true;            
              System.err.println(e.getMessage());          
          }           
          try {
              if ("".equals(row13.geographyCoef)){  
                row13.geographyCoef = null;
              }
              row2.geographyCoef=TypeConvert.String2float(row13.geographyCoef);            
          } catch(java.lang.Exception e){
            bHasError_tConvertType_1 = true;            
              System.err.println(e.getMessage());          
          }           
          try {
              if ("".equals(row13.splitNumber)){  
                row13.splitNumber = null;
              }
              row2.splitNumber=TypeConvert.String2float(row13.splitNumber);            
          } catch(java.lang.Exception e){
            bHasError_tConvertType_1 = true;            
              System.err.println(e.getMessage());          
          }           
          try {
              if ("".equals(row13.attribute1)){  
                row13.attribute1 = null;
              }
              row2.attribute1=TypeConvert.String2String(row13.attribute1);            
          } catch(java.lang.Exception e){
            bHasError_tConvertType_1 = true;            
              System.err.println(e.getMessage());          
          }           
          try {
              if ("".equals(row13.attribute2)){  
                row13.attribute2 = null;
              }
              row2.attribute2=TypeConvert.String2String(row13.attribute2);            
          } catch(java.lang.Exception e){
            bHasError_tConvertType_1 = true;            
              System.err.println(e.getMessage());          
          }           
          try {
              if ("".equals(row13.attribute3)){  
                row13.attribute3 = null;
              }
              row2.attribute3=TypeConvert.String2String(row13.attribute3);            
          } catch(java.lang.Exception e){
            bHasError_tConvertType_1 = true;            
              System.err.println(e.getMessage());          
          }           
          try {
              if ("".equals(row13.attribute4)){  
                row13.attribute4 = null;
              }
              row2.attribute4=TypeConvert.String2String(row13.attribute4);            
          } catch(java.lang.Exception e){
            bHasError_tConvertType_1 = true;            
              System.err.println(e.getMessage());          
          }           
          try {
              if ("".equals(row13.attribute5)){  
                row13.attribute5 = null;
              }
              row2.attribute5=TypeConvert.String2String(row13.attribute5);            
          } catch(java.lang.Exception e){
            bHasError_tConvertType_1 = true;            
              System.err.println(e.getMessage());          
          }
 					    ...
 					    boolean forceLooprow5 = false;
 							...
 							row5Struct row5ObjectFromLookup = null;
								...
								hasCasePrimitiveKeyWithNull_tMap_2 = false;
                        		    		...
                        		    		row5HashKey.businessKey = row2.providerCode ;
...
row8_tmp.geographyCoef = row2.geographyCoef ;
row8_tmp.splitNumber = row2.splitNumber ;
...
if(row8 != null) { 
	...
	row10.providerCode = row8.providerCode;
	...
	row10.geographyCoef = row8.geographyCoef;

	
	row10.splitNumber = row8.splitNumber;
 ...
 */
	...
	if (rsvUtil_tSchemaComplianceCheck_1.ifPassedThrough) {
		...
		row11.providerCode = row9.providerCode;
		row11.geographyCode = row9.geographyCode;
		row11.sourceChannelCode = row9.sourceChannelCode;
		row11.nameEn = row9.nameEn;
		row11.nameLo = row9.nameLo;
		row11.geographyCoef = row9.geographyCoef;
		row11.splitNumber = row9.splitNumber;
	...
	if (!rsvUtil_tSchemaComplianceCheck_1.ifPassedThrough) {
		...
		exception1.providerCode = row9.providerCode;
		exception1.geographyCode = row9.geographyCode;
		exception1.sourceChannelCode = row9.sourceChannelCode;
		exception1.nameEn = row9.nameEn;
		exception1.nameLo = row9.nameLo;
		exception1.geographyCoef = row9.geographyCoef;
		exception1.splitNumber = row9.splitNumber;
...
pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
                    ...
                    if(row11.providerCode == null) {
pstmt_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(3, row11.providerCode);
}

                    if(row11.geographyCode == null) {
pstmt_tDBOutput_1.setNull(4, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(4, row11.geographyCode);
}

                    if(row11.sourceChannelCode == null) {
pstmt_tDBOutput_1.setNull(5, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(5, row11.sourceChannelCode);
}

                    if(row11.nameEn == null) {
pstmt_tDBOutput_1.setNull(6, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(6, row11.nameEn);
}

                    if(row11.nameLo == null) {
pstmt_tDBOutput_1.setNull(7, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(7, row11.nameLo);
}

                    pstmt_tDBOutput_1.setFloat(8, row11.geographyCoef);

                    pstmt_tDBOutput_1.setFloat(9, row11.splitNumber);
 ...
 */
    ...
    public static void main(String[] args){
        final SAT_CUSTOMER SAT_CUSTOMERClass = new SAT_CUSTOMER();

        int exitCode = SAT_CUSTOMERClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = SAT_CUSTOMER.class.getClassLoader().getResourceAsStream("deepsea/sat_customer_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = SAT_CUSTOMER.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : SAT_CUSTOMER");
        }
...
/************************************************************************************************
 *     311750 characters generated by Talend Open Studio for Big Data 
 *     on the 4 February, 2019 11:00:02 AM SGT
 ************************************************************************************************/