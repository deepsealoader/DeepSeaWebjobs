				...
				try{
					...
					for (com.mongodb.DBObject index: coll_tMongoDBInput_1.listIndexes(com.mongodb.DBObject.class)) {
	                    ...
	                    for (String key: ((com.mongodb.DBObject)index.get("key")).keySet()) {
	                        ...
	                        // - is followed at some point by a colon
	                        // - there is no comma between the the DBcolumnName and the colon
	                        if  (("{'file.$id': ObjectId('" + globalMap.get("row4.id").toString() + "')}, {'_id':1, 'num':1, 'fields':1}").matches(".*" + key + "[^,]*:.*")) {
	                            // We have an index, do not print error message
	                            needIndexWarning = false;
	                        } else {
	                            // This index is not in the query, add it into the indexList
					...
					org.bson.Document myQuery_tMongoDBInput_1 = org.bson.Document.parse("{'file.$id': ObjectId('" + globalMap.get("row4.id").toString() + "')}, {'_id':1, 'num':1, 'fields':1}");
 					    ...
 					    boolean forceLooprow5 = false;
 							...
 							row5Struct row5ObjectFromLookup = null;
								...
								hasCasePrimitiveKeyWithNull_tMap_2 = false;
                        		    		...
                        		    		row5HashKey.hash = globalMap.get("row4.hash").toString();
...
Var.date = (row2.date != null) ? row2.date : ("1/" + row2.dateMonth + "/" + row2.dateYear) ;// ###############################
        // ###############################
        // # Output tables
...
row8_tmp.expiryDate = row2.expiryDate != null ? TalendDate.parseDate("yyyy-MM-dd", Clean.cleanDate(row2.expiryDate, null, context.DEFAULT_DATE_FORMAT)) : null ;
...
/************************************************************************************************
 *     365783 characters generated by Talend Open Studio for Big Data 
 *     on the 15 August, 2019 7:39:54 PM SGT
 ************************************************************************************************/