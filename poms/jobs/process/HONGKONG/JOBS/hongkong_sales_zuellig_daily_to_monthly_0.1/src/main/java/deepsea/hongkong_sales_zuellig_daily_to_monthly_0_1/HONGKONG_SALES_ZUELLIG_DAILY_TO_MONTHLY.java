...
package deepsea.hongkong_sales_zuellig_daily_to_monthly_0_1;
...
/**
 * Job: HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 7.0.1.20180411_1414
 * @status 
 */
public class HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY implements TalendJob {
...
public String getPOSTGRES_HOSTNAME(){
	...
	private final String jobName = "HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY";
...
private RunStat runStat = new RunStat();
	...
	MetterCatcherUtils tFlowMeterCatcher_1 = new MetterCatcherUtils("_lLLuoBPfEemc7a44kBSjTQ", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
			...
			public void tFlowMeter_12_error(Exception exception, String errorComponent, final java.util.Map<String, Object> globalMap) throws TalendException {
...
public static class row10Struct implements routines.system.IPersistableRow<row10Struct> {
    final static byte[] commonByteArrayLock_DEEPSEA_HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY = new byte[0];
    static byte[] commonByteArray_DEEPSEA_HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_DEEPSEA_HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY.length) {
				if(length < 1024 && commonByteArray_DEEPSEA_HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY.length == 0) {
   					commonByteArray_DEEPSEA_HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY = new byte[1024];
				} else {
   					commonByteArray_DEEPSEA_HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_DEEPSEA_HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY, 0, length);
			strReturn = new String(commonByteArray_DEEPSEA_HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_DEEPSEA_HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY) {
	...
	int nb_line_tMongoDBInput_3 = 0;
			...
			com.mongodb.client.MongoDatabase db_tMongoDBInput_3=null;
				...
				db_tMongoDBInput_3 = (com.mongodb.client.MongoDatabase) globalMap.get("db_tMongoDBConnection_1");
					...
					com.mongodb.client.MongoCollection<org.bson.Document> coll_tMongoDBInput_3 = db_tMongoDBInput_3.getCollection("countries");
				...
				try{
					...
					org.bson.Document myQuery_tMongoDBInput_3 = org.bson.Document.parse("{'name': '" + context.getCOUNTRY() + "'}, {'_id':1}");
				...
				if(valueObj_tMongoDBInput_3!=null && valueObj_tMongoDBInput_3.toString().length() > 0) {
					row10.id = valueObj_tMongoDBInput_3.toString();
				}else{
					row10.id = null;
				}
	...
	int nb_line_tMongoDBInput_4 = 0;
			...
			com.mongodb.client.MongoDatabase db_tMongoDBInput_4=null;
				...
				try{
					...
					for (com.mongodb.DBObject index: coll_tMongoDBInput_4.listIndexes(com.mongodb.DBObject.class)) {
	                    ...
	                    for (String key: ((com.mongodb.DBObject)index.get("key")).keySet()) {
	                        ...
	                        // - is followed at some point by a colon
	                        // - there is no comma between the the DBcolumnName and the colon
	                        if  (("{'country.$id': ObjectId('" + globalMap.get("COUNTRY_ID").toString() + "'),'name': '" + context.getCOLLECTION1() + "'}, {'_id':1}").matches(".*" + key + "[^,]*:.*")) {
	                            // We have an index, do not print error message
	                            needIndexWarning = false;
	                        } else {
	                            // This index is not in the query, add it into the indexList
					...
					org.bson.Document myQuery_tMongoDBInput_4 = org.bson.Document.parse("{'country.$id': ObjectId('" + globalMap.get("COUNTRY_ID").toString() + "'),'name': '" + context.getCOLLECTION1() + "'}, {'_id':1}");
				...
				if(valueObj_tMongoDBInput_4!=null && valueObj_tMongoDBInput_4.toString().length() > 0) {
					row5.id = valueObj_tMongoDBInput_4.toString();
				}else{
					row5.id = null;
				}
...
globalMap.put("COLLECTION1_ID", row5.id);
	...
	int nb_line_tMongoDBInput_5 = 0;
			...
			com.mongodb.client.MongoDatabase db_tMongoDBInput_5=null;
				...
				try{
					...
					for (com.mongodb.DBObject index: coll_tMongoDBInput_5.listIndexes(com.mongodb.DBObject.class)) {
	                    ...
	                    for (String key: ((com.mongodb.DBObject)index.get("key")).keySet()) {
	                        ...
	                        // - is followed at some point by a colon
	                        // - there is no comma between the the DBcolumnName and the colon
	                        if  (("{'country.$id': ObjectId('" + globalMap.get("COUNTRY_ID").toString() + "'),'name': '" + context.getCOLLECTION2() + "'}, {'_id':1}").matches(".*" + key + "[^,]*:.*")) {
	                            // We have an index, do not print error message
	                            needIndexWarning = false;
	                        } else {
	                            // This index is not in the query, add it into the indexList
					...
					org.bson.Document myQuery_tMongoDBInput_5 = org.bson.Document.parse("{'country.$id': ObjectId('" + globalMap.get("COUNTRY_ID").toString() + "'),'name': '" + context.getCOLLECTION2() + "'}, {'_id':1}");
				...
				if(valueObj_tMongoDBInput_5!=null && valueObj_tMongoDBInput_5.toString().length() > 0) {
					row7.id = valueObj_tMongoDBInput_5.toString();
				}else{
					row7.id = null;
				}
...
globalMap.put("COLLECTION2_ID", row7.id);
...
String fileName_tFileOutputDelimited_1 = "";
    fileName_tFileOutputDelimited_1 = (new java.io.File(globalMap.get("tCreateTemporaryFile_1_FILEPATH").toString())).getAbsolutePath().replace("\\","/");
	...
	int nb_line_tMongoDBInput_2 = 0;
			...
			com.mongodb.client.MongoDatabase db_tMongoDBInput_2=null;
				...
				db_tMongoDBInput_2 = (com.mongodb.client.MongoDatabase) globalMap.get("db_tMongoDBConnection_1");
					...
					com.mongodb.client.MongoCollection<org.bson.Document> coll_tMongoDBInput_2 = db_tMongoDBInput_2.getCollection("transactions.files");
				...
				try{
	            	...
	            	// maybe due to authorization
							...
							aggregationStages.add(org.bson.Document.parse("{ $match : {'country.$id': ObjectId('" + globalMap.get("COUNTRY_ID").toString() + "'), 'collection.$id' : ObjectId('" + globalMap.get("COLLECTION1_ID").toString() + "')} }"));
						
	                	
	                
	                	
							com.mongodb.client.MongoCursor<org.bson.Document> cursor_tMongoDBInput_2 = coll_tMongoDBInput_2.aggregate(aggregationStages).allowDiskUse(false).iterator();
				...
				pathMap_tMongoDBInput_2.put("batchDate","");
				pathMap_tMongoDBInput_2.put("hash","");
				...
				Object valueObj_tMongoDBInput_2=null;
                    valueObj_tMongoDBInput_2=dbObjectInputUtil_tMongoDBInput_2.getValue(pathMap_tMongoDBInput_2.get("batchDate"),"batchDate",o_tMongoDBInput_2);
				if(valueObj_tMongoDBInput_2!=null && valueObj_tMongoDBInput_2.toString().length() > 0) {
                            if (valueObj_tMongoDBInput_2 instanceof java.util.Date) {
																																row12.batchDate = ParserUtils.parseTo_Date((java.util.Date)valueObj_tMongoDBInput_2, "dd-MM-yyyy");
                            } else {
                                row12.batchDate = ParserUtils.parseTo_Date((String)valueObj_tMongoDBInput_2, "dd-MM-yyyy");
                            }
				}else{
					row12.batchDate = null;
				}
                    valueObj_tMongoDBInput_2=dbObjectInputUtil_tMongoDBInput_2.getValue(pathMap_tMongoDBInput_2.get("hash"),"hash",o_tMongoDBInput_2);
				if(valueObj_tMongoDBInput_2!=null && valueObj_tMongoDBInput_2.toString().length() > 0) {
					row12.hash = valueObj_tMongoDBInput_2.toString();
				}else{
					row12.hash = null;
				}
				...
				if(execStat){
					runStat.updateStatOnConnection("row12"+iterateId,1, 1);
                        ...
                        String[] rowtFileOutputDelimited_1=new String[2];
                            rowtFileOutputDelimited_1[0]=row12.batchDate == null ? null : FormatterUtils.format_Date(row12.batchDate, "dd-MM-yyyy");
                            rowtFileOutputDelimited_1[1]=row12.hash == null ? null : row12.hash;
 ...
 */
							...
							tMongoDBInput_8Process(globalMap); 
...
String fileName_tFileOutputDelimited_2 = "";
    fileName_tFileOutputDelimited_2 = (new java.io.File(globalMap.get("tCreateTemporaryFile_2_FILEPATH").toString())).getAbsolutePath().replace("\\","/");
 ...
 */
	...
	currentComponent="tMongoDBInput_8";
    	...
    	class BytesLimit65535_tMongoDBInput_8{
    		public void limitLog4jByte() throws Exception{
    			
    		}
    	}
    	
        new BytesLimit65535_tMongoDBInput_8().limitLog4jByte();

	int nb_line_tMongoDBInput_8 = 0;
	
		
		
			com.mongodb.MongoClient mongo_tMongoDBInput_8=null;
			com.mongodb.client.MongoDatabase db_tMongoDBInput_8=null;
				...
				mongo_tMongoDBInput_8=(com.mongodb.MongoClient)globalMap.get("mongo_tMongoDBConnection_1");
				db_tMongoDBInput_8 = (com.mongodb.client.MongoDatabase) globalMap.get("db_tMongoDBConnection_1");
				


				
				
					com.mongodb.client.MongoCollection<org.bson.Document> coll_tMongoDBInput_8 = db_tMongoDBInput_8.getCollection("transactions.records");
				...
				try{
	            	...
	            	// maybe due to authorization
							...
							aggregationStages.add(org.bson.Document.parse("{$project: {'file': {$arrayElemAt: [{$filter: {input: { $objectToArray: '$file' }, cond: { $eq: [ '$$this.k', {$literal:'$id'} ] } } }, 0 ]}, 'fields': {$arrayToObject: {$filter: {input: {$map: {input: '$fields', in: { 'k': '$$this.tag', 'v': '$$this.value' } } }, cond: { $eq: [ '$$this.k', 'hash' ] } } } }, '_id': 0 } }"));
						
	                	
	                
	                	
							aggregationStages.add(org.bson.Document.parse("{$match: { 'fields.hash': {$ne: null} } }"));
						
	                	
	                
	                	
							aggregationStages.add(org.bson.Document.parse("{$group: { _id : { 'file_id': '$file.v', 'hash': '$fields.hash' } } }"));
						
	                	
	                
	                	
							aggregationStages.add(org.bson.Document.parse("{$lookup: {from: 'transactions.files', localField: '_id.file_id', foreignField: '_id', as: 'file'} }"));
						
	                	
	                
	                	
							aggregationStages.add(org.bson.Document.parse("{$match: {'file.country.$id': ObjectId('" + globalMap.get("COUNTRY_ID").toString() + "'), 'file.collection.$id' : ObjectId('" + globalMap.get("COLLECTION2_ID").toString() + "') } }"));
						
	                	
	                
	                	
							aggregationStages.add(org.bson.Document.parse("{$group: { _id : { 'batchDate': { $arrayElemAt: [ '$file.batchDate', 0] }, 'hash': '$_id.hash' } } }"));
						
	                	
	                
	                	
							aggregationStages.add(org.bson.Document.parse("{$project: { _id: 0, 'batchDate': '$_id.batchDate', 'hash': '$_id.hash' } }"));
						
	                	
	                
	                	
							com.mongodb.client.MongoCursor<org.bson.Document> cursor_tMongoDBInput_8 = coll_tMongoDBInput_8.aggregate(aggregationStages).allowDiskUse(false).iterator();
				...
				java.util.Map<String, String> pathMap_tMongoDBInput_8=new java.util.HashMap<String, String>();
				pathMap_tMongoDBInput_8.put("batchDate","");
				pathMap_tMongoDBInput_8.put("hash","");
				...
				Object valueObj_tMongoDBInput_8=null;
                    valueObj_tMongoDBInput_8=dbObjectInputUtil_tMongoDBInput_8.getValue(pathMap_tMongoDBInput_8.get("batchDate"),"batchDate",o_tMongoDBInput_8);
				if(valueObj_tMongoDBInput_8!=null && valueObj_tMongoDBInput_8.toString().length() > 0) {
                            if (valueObj_tMongoDBInput_8 instanceof java.util.Date) {
																																row17.batchDate = ParserUtils.parseTo_Date((java.util.Date)valueObj_tMongoDBInput_8, "dd-MM-yyyy");
                            } else {
                                row17.batchDate = ParserUtils.parseTo_Date((String)valueObj_tMongoDBInput_8, "dd-MM-yyyy");
                            }
				}else{
					row17.batchDate = null;
				}
                    valueObj_tMongoDBInput_8=dbObjectInputUtil_tMongoDBInput_8.getValue(pathMap_tMongoDBInput_8.get("hash"),"hash",o_tMongoDBInput_8);
				if(valueObj_tMongoDBInput_8!=null && valueObj_tMongoDBInput_8.toString().length() > 0) {
					row17.hash = valueObj_tMongoDBInput_8.toString();
				}else{
					row17.hash = null;
				}
				...
				if(execStat){
					runStat.updateStatOnConnection("row17"+iterateId,1, 1);
                        ...
                        String[] rowtFileOutputDelimited_2=new String[2];
                            rowtFileOutputDelimited_2[0]=row17.batchDate == null ? null : FormatterUtils.format_Date(row17.batchDate, "dd-MM-yyyy");
                            rowtFileOutputDelimited_2[1]=row17.hash == null ? null : row17.hash;
                synchronized (multiThreadLockWrite) {
 ...
 */
				    			...
				    			if(resumeEntryMethodName == null || globalResumeTicket){
				    				resumeUtil.addLog("CHECKPOINT", "CONNECTION:SUBJOB_OK:tMongoDBInput_8:OnSubjobOk", "", Thread.currentThread().getId() + "", "", "", "", "", "");
								}	    				    			
		...
		public int hashCode() {
			if (this.hashCodeDirty) {
				...
				int result = DEFAULT_HASHCODE;
							...
							result = prime * result + ((this.batchDate == null) ? 0 : this.batchDate.hashCode());
 ...
 */
	...
	final routines.system.RowState rowstate_tFileInputDelimited_3 = new routines.system.RowState();
				...
				Object filename_tFileInputDelimited_3 = /** Start field tFileInputDelimited_3:FILENAME */globalMap.get("tCreateTemporaryFile_1_FILEPATH").toString()/** End field tFileInputDelimited_3:FILENAME */;		
				...
				if(rowtFileInputDelimited_3.length == 1 && ("\015").equals(rowtFileInputDelimited_3[0])){//empty line when row separator is '\n'
	                ...
	                int columnIndexWithD_tFileInputDelimited_3 = 0; //Column Index 
						...
						if(columnIndexWithD_tFileInputDelimited_3 < rowtFileInputDelimited_3.length){
									...
									if(rowtFileInputDelimited_3[columnIndexWithD_tFileInputDelimited_3].length() > 0) {
										try {
											...
											row1.batchDate = ParserUtils.parseTo_Date(rowtFileInputDelimited_3[columnIndexWithD_tFileInputDelimited_3], "dd-MM-yyyy");
											...
											rowstate_tFileInputDelimited_3.setException(new RuntimeException(String.format("Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
												"batchDate", "row1", rowtFileInputDelimited_3[columnIndexWithD_tFileInputDelimited_3], ex_tFileInputDelimited_3), ex_tFileInputDelimited_3));
										}
    								}else{
						...
						if(columnIndexWithD_tFileInputDelimited_3 < rowtFileInputDelimited_3.length){
									...
									row1.hash = rowtFileInputDelimited_3[columnIndexWithD_tFileInputDelimited_3];
 					    ...
 					    boolean forceLooprow3 = false;
 							...
 							row3Struct row3ObjectFromLookup = null;
								...
								hasCasePrimitiveKeyWithNull_tMap_1 = false;
                        		    		...
                        		    		row3HashKey.batchDate = row1.batchDate ;
                        		    		
                        		    		row3HashKey.hash = row1.hash ;
								  ...
								  if(!tHash_Lookup_row3.hasNext()) { // G_TM_M_090
				           		...
				           		if(tHash_Lookup_row3 != null && tHash_Lookup_row3.getCount(row3HashKey) > 1) { // G 071
									...
									//System.out.println("WARNING: UNIQUE MATCH is configured for the lookup 'row3' and it contains more one result from keys :  row3.batchDate = '" + row3HashKey.batchDate + "', row3.hash = '" + row3HashKey.hash + "'");
								} // G 071
...
// # Filter conditions 
if( rejectedInnerJoin_tMap_1 ) {
newRow_tmp.batchDate = row1.batchDate;
newRow = newRow_tmp;
...
// for output
			...
			row14.batchDate = row9.batchDate;			
...
operation_finder_tAggregateRow_2.batchDate = row14.batchDate;
	...
	if(operation_result_tAggregateRow_2 == null) { // G_OutMain_AggR_001
		...
		operation_result_tAggregateRow_2.batchDate = operation_finder_tAggregateRow_2.batchDate;
 ...
 */
	...
	tFlowMeterCatcher_1.addMessage("newRow", new Integer(count_tFlowMeter_12), "null", "", "tFlowMeter_12");
 ...
 */
	...
	final routines.system.RowState rowstate_tFileInputDelimited_1 = new routines.system.RowState();
				...
				Object filename_tFileInputDelimited_1 = /** Start field tFileInputDelimited_1:FILENAME */globalMap.get("tCreateTemporaryFile_2_FILEPATH").toString()/** End field tFileInputDelimited_1:FILENAME */;		
				...
				if(rowtFileInputDelimited_1.length == 1 && ("\015").equals(rowtFileInputDelimited_1[0])){//empty line when row separator is '\n'
	                ...
	                int columnIndexWithD_tFileInputDelimited_1 = 0; //Column Index 
						...
						if(columnIndexWithD_tFileInputDelimited_1 < rowtFileInputDelimited_1.length){
									...
									if(rowtFileInputDelimited_1[columnIndexWithD_tFileInputDelimited_1].length() > 0) {
										try {
											...
											row2.batchDate = ParserUtils.parseTo_Date(rowtFileInputDelimited_1[columnIndexWithD_tFileInputDelimited_1], "dd-MM-yyyy");
										
									
										} catch(java.lang.Exception ex_tFileInputDelimited_1) {
											rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format("Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
												"batchDate", "row2", rowtFileInputDelimited_1[columnIndexWithD_tFileInputDelimited_1], ex_tFileInputDelimited_1), ex_tFileInputDelimited_1));
										}
    								}else{
						...
						if(columnIndexWithD_tFileInputDelimited_1 < rowtFileInputDelimited_1.length){
									...
									row2.hash = rowtFileInputDelimited_1[columnIndexWithD_tFileInputDelimited_1];
 					    ...
 					    boolean forceLooprow4 = false;
 							...
 							row4Struct row4ObjectFromLookup = null;
								...
								hasCasePrimitiveKeyWithNull_tMap_2 = false;
                        		    		...
                        		    		row4HashKey.batchDate = row2.batchDate ;
                        		    		
                        		    		row4HashKey.hash = row2.hash ;
								  ...
								  if(!tHash_Lookup_row4.hasNext()) { // G_TM_M_090
				           		...
				           		if(tHash_Lookup_row4 != null && tHash_Lookup_row4.getCount(row4HashKey) > 1) { // G 071
									...
									//System.out.println("WARNING: UNIQUE MATCH is configured for the lookup 'row4' and it contains more one result from keys :  row4.batchDate = '" + row4HashKey.batchDate + "', row4.hash = '" + row4HashKey.hash + "'");
								} // G 071
...
// # Output reject table : 'orgRow'
// # Filter conditions 
if( rejectedInnerJoin_tMap_2 ) {
orgRow_tmp.batchDate = row2.batchDate ;
orgRow = orgRow_tmp;
} // closing filter/reject
// ###############################
...
// for output
			...
			row14.batchDate = row11.batchDate;			
 ...
 */
	...
	tFlowMeterCatcher_1.addMessage("orgRow", new Integer(count_tFlowMeter_19), "null", "", "tFlowMeter_19");
 ...
 */
	...
	currentComponent="tAggregateRow_2_AGGIN";
            				    ...
            				    row8.batchDate = aggregated_row_tAggregateRow_2.batchDate;
 ...
 */
	...
	java.util.Map<String, Object> parentContextMap_tRunJob_1 = new java.util.HashMap<String, Object>();
		...
		context.synchronizeContext();
			...
			parentContextMap_tRunJob_1.put("COLLECTION1", context.COLLECTION1);
			paraList_tRunJob_1.add("--context_type " + "COLLECTION1" + "=" + "id_String");
		
			parentContextMap_tRunJob_1.put("COLLECTION2", context.COLLECTION2);
			paraList_tRunJob_1.add("--context_type " + "COLLECTION2" + "=" + "id_String");
	...
	Object obj_tRunJob_1 = null;
		...
		obj_tRunJob_1 = TalendDate.formatDate("yyyy-MM-dd", row8.batchDate);
		if(obj_tRunJob_1!=null) {
			paraList_tRunJob_1.add("--context_param BATCH_DATE=" + RuntimeUtils.tRunJobConvertContext(obj_tRunJob_1));
		} else {
			paraList_tRunJob_1.add("--context_param BATCH_DATE=" + NULL_VALUE_EXPRESSION_IN_COMMAND_STRING_FOR_CHILD_JOB_ONLY);
		}
		
		parentContextMap_tRunJob_1.put("BATCH_DATE", obj_tRunJob_1);
	
		obj_tRunJob_1 = context.getCOLLECTION1();
		if(obj_tRunJob_1!=null) {
			paraList_tRunJob_1.add("--context_param COLLECTION1=" + RuntimeUtils.tRunJobConvertContext(obj_tRunJob_1));
		} else {
			paraList_tRunJob_1.add("--context_param COLLECTION1=" + NULL_VALUE_EXPRESSION_IN_COMMAND_STRING_FOR_CHILD_JOB_ONLY);
		}
		
		parentContextMap_tRunJob_1.put("COLLECTION1", obj_tRunJob_1);
	
		obj_tRunJob_1 = context.getCOLLECTION2();
		if(obj_tRunJob_1!=null) {
			paraList_tRunJob_1.add("--context_param COLLECTION2=" + RuntimeUtils.tRunJobConvertContext(obj_tRunJob_1));
		} else {
			paraList_tRunJob_1.add("--context_param COLLECTION2=" + NULL_VALUE_EXPRESSION_IN_COMMAND_STRING_FOR_CHILD_JOB_ONLY);
		}
		
		parentContextMap_tRunJob_1.put("COLLECTION2", obj_tRunJob_1);
	
		obj_tRunJob_1 = context.getCOLLECTION2() + "-";
		if(obj_tRunJob_1!=null) {
			paraList_tRunJob_1.add("--context_param FILE_PREFIX=" + RuntimeUtils.tRunJobConvertContext(obj_tRunJob_1));
		} else {
			paraList_tRunJob_1.add("--context_param FILE_PREFIX=" + NULL_VALUE_EXPRESSION_IN_COMMAND_STRING_FOR_CHILD_JOB_ONLY);
		}
		
		parentContextMap_tRunJob_1.put("FILE_PREFIX", obj_tRunJob_1);
	
		obj_tRunJob_1 = "yyyyMMdd";
		if(obj_tRunJob_1!=null) {
			paraList_tRunJob_1.add("--context_param DATE_FORMAT=" + RuntimeUtils.tRunJobConvertContext(obj_tRunJob_1));
		} else {
			paraList_tRunJob_1.add("--context_param DATE_FORMAT=" + NULL_VALUE_EXPRESSION_IN_COMMAND_STRING_FOR_CHILD_JOB_ONLY);
		}
		
		parentContextMap_tRunJob_1.put("DATE_FORMAT", obj_tRunJob_1);
	
	
		deepsea.hongkong_update_one_month_sales_0_1.HONGKONG_UPDATE_ONE_MONTH_SALES childJob_tRunJob_1 = new deepsea.hongkong_update_one_month_sales_0_1.HONGKONG_UPDATE_ONE_MONTH_SALES();
	    // pass DataSources
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_row3";
        ...
        new BytesLimit65535_tAdvancedHash_row3().limitLog4jByte();
			   		...
			   		// source node:tFileInputDelimited_4 - inputs:(after_tFileInputDelimited_3) outputs:(row3,row3) | target node:tAdvancedHash_row3 - inputs:(row3) outputs:()
			   		// linked node: tMap_1 - inputs:(row1,row3) outputs:(newRow)
 ...
 */
	...
	final routines.system.RowState rowstate_tFileInputDelimited_4 = new routines.system.RowState();
				...
				Object filename_tFileInputDelimited_4 = /** Start field tFileInputDelimited_4:FILENAME */globalMap.get("tCreateTemporaryFile_2_FILEPATH").toString()/** End field tFileInputDelimited_4:FILENAME */;		
				...
				if(rowtFileInputDelimited_4.length == 1 && ("\015").equals(rowtFileInputDelimited_4[0])){//empty line when row separator is '\n'
	                ...
	                int columnIndexWithD_tFileInputDelimited_4 = 0; //Column Index 
						...
						if(columnIndexWithD_tFileInputDelimited_4 < rowtFileInputDelimited_4.length){
									...
									if(rowtFileInputDelimited_4[columnIndexWithD_tFileInputDelimited_4].length() > 0) {
										try {
											...
											row3.batchDate = ParserUtils.parseTo_Date(rowtFileInputDelimited_4[columnIndexWithD_tFileInputDelimited_4], "dd-MM-yyyy");
											...
											rowstate_tFileInputDelimited_4.setException(new RuntimeException(String.format("Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
												"batchDate", "row3", rowtFileInputDelimited_4[columnIndexWithD_tFileInputDelimited_4], ex_tFileInputDelimited_4), ex_tFileInputDelimited_4));
										}
    								}else{
						...
						if(columnIndexWithD_tFileInputDelimited_4 < rowtFileInputDelimited_4.length){
									...
									row3.hash = rowtFileInputDelimited_4[columnIndexWithD_tFileInputDelimited_4];
...
if(row3 != null) { 
	...
	currentComponent="tAdvancedHash_row3";
			...
			//row3
				...
				row3_HashRow.batchDate = row3.batchDate;
				
				row3_HashRow.hash = row3.hash;
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_row4";
        ...
        new BytesLimit65535_tAdvancedHash_row4().limitLog4jByte();
			   		...
			   		// linked node: tMap_2 - inputs:(row2,row4) outputs:(orgRow)
 ...
 */
	...
	final routines.system.RowState rowstate_tFileInputDelimited_2 = new routines.system.RowState();
				...
				Object filename_tFileInputDelimited_2 = /** Start field tFileInputDelimited_2:FILENAME */globalMap.get("tCreateTemporaryFile_1_FILEPATH").toString()/** End field tFileInputDelimited_2:FILENAME */;		
				...
				if(rowtFileInputDelimited_2.length == 1 && ("\015").equals(rowtFileInputDelimited_2[0])){//empty line when row separator is '\n'
	                ...
	                int columnIndexWithD_tFileInputDelimited_2 = 0; //Column Index 
						...
						if(columnIndexWithD_tFileInputDelimited_2 < rowtFileInputDelimited_2.length){
									...
									if(rowtFileInputDelimited_2[columnIndexWithD_tFileInputDelimited_2].length() > 0) {
										try {
											...
											row4.batchDate = ParserUtils.parseTo_Date(rowtFileInputDelimited_2[columnIndexWithD_tFileInputDelimited_2], "dd-MM-yyyy");
											...
											rowstate_tFileInputDelimited_2.setException(new RuntimeException(String.format("Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
												"batchDate", "row4", rowtFileInputDelimited_2[columnIndexWithD_tFileInputDelimited_2], ex_tFileInputDelimited_2), ex_tFileInputDelimited_2));
										}
    								}else{
						...
						if(columnIndexWithD_tFileInputDelimited_2 < rowtFileInputDelimited_2.length){
									...
									row4.hash = rowtFileInputDelimited_2[columnIndexWithD_tFileInputDelimited_2];
 ...
 */
    ...
    public static void main(String[] args){
        final HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLYClass = new HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY();

        int exitCode = HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLYClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY.class.getClassLoader().getResourceAsStream("deepsea/hongkong_sales_zuellig_daily_to_monthly_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
        ...
        if (parentContextMap != null && !parentContextMap.isEmpty()) {if (parentContextMap.containsKey("COLLECTION1")) {
...
final Thread launchingThread = Thread.currentThread();
        ...
        new Thread(){
            public void run() {
                ...
                try {
((java.util.Map) threadLocal.get()).put("errorCode", null);tMongoDBInput_2Process(globalMap);
...
}catch (java.lang.Error e_tMongoDBInput_2) {
globalMap.put("tMongoDBInput_2_SUBPROCESS_STATE", -1);
((java.util.Map) threadLocal.get()).put("status", "failure");throw e_tMongoDBInput_2;
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : HONGKONG_SALES_ZUELLIG_DAILY_TO_MONTHLY");
        }
...
/************************************************************************************************
 *     317287 characters generated by Talend Open Studio for Big Data 
 *     on the January 20, 2019 2:20:52 PM CST
 ************************************************************************************************/