...
public class SAT_SOURCE_CHANNEL implements TalendJob {
				...
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(SAT_SOURCE_CHANNEL.this, new Object[] { e,
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_DEEPSEA_SAT_SOURCE_CHANNEL.length) {
					if (length < 1024
							&& commonByteArray_DEEPSEA_SAT_SOURCE_CHANNEL.length == 0) {
						commonByteArray_DEEPSEA_SAT_SOURCE_CHANNEL = new byte[1024];
					} else {
						commonByteArray_DEEPSEA_SAT_SOURCE_CHANNEL = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_DEEPSEA_SAT_SOURCE_CHANNEL, 0,
						length);
				strReturn = new String(
						commonByteArray_DEEPSEA_SAT_SOURCE_CHANNEL, 0, length,
						utf8Charset);
			}
			return strReturn;
		}
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String insert_tDBOutput_1 = "INSERT INTO \""
						...
						+ "\" (\"hub_id\",\"source\",\"code\",\"name\") VALUES (?,?,?,?)";
				...
				aggregationStages
						.add(org.bson.Document
								.parse("{ $lookup: {from: 'master.files', localField: '_fileRef.id', foreignField: '_id', as: '_file'} }"));
				...
				aggregationStages.add(org.bson.Document
						.parse("{ $match : {'file.metadata.type' : '"
				...
				while (cursor_tMongoDBInput_1.hasNext()) {
					...
					if (out1 != null) {
						...
						row3.servierCode = utility_tExtractFields_1.UnPivot(
								doc_tExtractFields_1, "tag", "value",
								"servierCode");
						row3.name = utility_tExtractFields_1.UnPivot(
								doc_tExtractFields_1, "tag", "value", "name");
						...
						if (row3 != null) {
							...
							try {
								...
								row2.servierCode = TypeConvert
										.String2String(row3.servierCode);
							} catch (java.lang.Exception e) {
							...
							try {
								...
								row2.name = TypeConvert
										.String2String(row3.name);
							} catch (java.lang.Exception e) {
								bHasError_tConvertType_1 = true;
								System.err.println(e.getMessage());
							}
				...
				tFlowMeterCatcher_1.addMessage("row10", new Integer(
						count_tFlowMeter_13), "null", "", "tFlowMeter_13");
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = SAT_SOURCE_CHANNEL.class
					.getClassLoader().getResourceAsStream(
							"deepsea/sat_source_channel_0_1/contexts/"
									+ contextStr + ".properties");
			if (inContext == null) {
				inContext = SAT_SOURCE_CHANNEL.class
						.getClassLoader()
						.getResourceAsStream(
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory)
							+ " bytes memory increase when running : SAT_SOURCE_CHANNEL");
		}
...
/************************************************************************************************
 * 182152 characters generated by Talend Open Studio for Big Data on the October
 * 13, 2018 1:35:13 PM CST
 ************************************************************************************************/
